...
export default function Community({ auth, datosComunidad, tanques = [] }) {
    ...
    const bubbleData = datosComunidad.map((item, index) => ({
        ...
        tds: item.tds || null, // Valor de TDS
    ...
    const calculateCommunityHealth = () => {
        const validTanks = bubbleData.filter(tank => 
            tank.tds && tank.currentWater && tank.level
        );
        
        if (validTanks.length === 0) return { averageHealth: 50, titoState: 'normal' };
        
        const totalHealth = validTanks.reduce((sum, tank) => {
            const health = calculateTankHealth(tank.tds, tank.currentWater, tank.level);
            return sum + (health || 0);
        }, 0);
    ...
    console.log(`Estado de la comunidad: ${titoState}, Salud promedio: ${averageHealth.toFixed(1)}%, Tanques: ${totalTanks}`);
            ...
            <AuthenticatedLayout user={auth.user}>
                ...
                >
                    ...
                    {bubbleData.map((bubble, index) => {
                        const { style, position } = calculateRadialPosition(index, bubbleData.length);
                        return (
                            <div
                                key={bubble.id}
                                style={style}
